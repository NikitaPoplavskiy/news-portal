...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <script>
        function getNews() {
            $("#loading").show();
            if ($('#news_count').is(':empty') && $('#new_news_count').is(':empty')) {
                ajaxRequest();
            } else {
                $('#news_count').text("Всего новостей: ");
                $('#new_news_count').text("Новых новостей: ");
                ajaxRequest();
            }
        }
        ...
        function ajaxRequest() {
            var request = $.ajax({
                type: 'GET',
                url: "{{ route('admin.sources.getNews') }}",
                dataType: 'json',
                success: function(response) {                    
                    // var result = $.parseJSON(response);
                    // console.log(result.newsCount);
                    // console.log(response.newsCount);

                    if (response.newsCount) {
                        $('#news_count').text("Всего новостей: " + response.newsCount);
                    }

                    $('#new_news_count').text("Новых новостей: " + response.newNewsCount);
                    $('#answer_good').slideDown(1000);
                    $("#loading").hide();
                },
                fail: function() {
                    $('#answer_fail').slideDown(1000);
                    $("#loading").hide();
                }
            });
        }
